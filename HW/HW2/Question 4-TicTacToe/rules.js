...
function isValid(_str) {
	if((_str == 'A1') || 
		...
		(_str == 'C3') )
	{
		if (document.getElementById(_str).innerHTML == _str)
		{
			return true;
		}
	}
...
function position(_str)
{
	if(_str == 'A1') return 0;
	else if(_str == 'A2') return 1;
	else if(_str == 'A3') return 2;
	else if(_str == 'B1') return 3;
	else if(_str == 'B2') return 4;
	else if(_str == 'B3') return 5;
	else if(_str == 'C1') return 6;
	else if(_str == 'C2') return 7;
	else return 8;
}

function isWin(player)
{
	if ((board_state[0] == player && board_state[1] == player && board_state[2] == player) ||
		(board_state[3] == player && board_state[4] == player && board_state[5] == player) ||
		(board_state[6] == player && board_state[7] == player && board_state[8] == player) ||
		(board_state[0] == player && board_state[3] == player && board_state[6] == player) ||
		(board_state[1] == player && board_state[4] == player && board_state[7] == player) ||
		(board_state[2] == player && board_state[5] == player && board_state[8] == player) ||
...
function play() {
	...
	playtext = document.getElementById('move_text_id').value;
	if(game_started())
	{
		if(!isValid(playtext))
		{
			alert('Usage is not mandatory.');
		}
		else
		{	
			if(isFull())
			...
			if(whose_move()==1)
			{
				document.getElementById(playtext).innerHTML = 'X';
				board_state[position(playtext)]=1;
				if(isWin(1))
				{
					alert('Winner is: X');
					reset_play();
			...
			{
				document.getElementById(playtext).innerHTML = 'O';
				board_state[position(playtext)]=0;
				if(isWin(0))
				{
					alert('Winner is: O');
					reset_play();
